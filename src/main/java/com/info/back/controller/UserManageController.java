...
*/
	...
	public void operation(HttpServletRequest request,HttpServletResponse response) {
		...
		try {
			...
			if("1".equals(request.getParameter("option"))){//黑名单
				...
				SpringUtils.renderDwzResult(response, bool, bool ? "操作成功!" : "该用户存在未完成的订单，无法执行该操作!", DwzResult.CALLBACK_RELOADPAGE);
			}
		} catch (Exception e) {
	...
	public String userdetails (HttpServletRequest request,Model model){
		...
		model.addAttribute("operatorHtml", reportDataHtml != null && reportDataHtml.contains("tenant.51datakey.com") && StringUtils.isNotBlank(tokenHtml) ? operatorHtml : (reportDataHtml == null ? "" : reportDataHtml));//运营商
