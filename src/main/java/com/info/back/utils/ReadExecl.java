...
public class ReadExecl {
	...
	public List<UserBlack> getExcelInfo(String fileName, MultipartFile Mfile){
		...
		File file = new  File("/home/uploads");
		//创建一个目录 （它的路径名由当前 File 对象指定，包括任一必须的父路径。）
		if (!file.exists()) file.mkdirs();
		//新建一个文件
		File file1 = new File("/home/uploads" + new Date().getTime() + ".xlsx");
		//将上传的文件写入新建的文件中
		try {
			...
			for(int c = 0; c <this.totalCells; c++){
				...
				if (null != cell){
					if(c==0){
                        ...
                        customer.setUserPhone(cell.getStringCellValue());//客户手机号
					}else if(c==2){
						cell.setCellType(Cell.CELL_TYPE_STRING);
                        //DecimalFormat df = new DecimalFormat("#");
						customer.setIdNumber(cell.getStringCellValue());//客户身份证号
					}
				}
			}
			//添加客户
