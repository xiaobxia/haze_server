package com.info.risk.utils.yx;
...
public class RSA_1024_V2 {
	 ...
	 * @return
	 */
	public static final RSAPrivateKey getPrivateKey(final String modulus,
			final String exponent) {
		try {
			final RSAPrivateKeySpec keySpec = new RSAPrivateKeySpec(
	 ...
	 * @return
	 */
	public static final String encodeByPrivateKey(
			final RSAPrivateKey privateKey, final String plainText) {
		try {
	 ...
	 * @return
	 */
	public static final String decodeByPrivateKey(
			final RSAPrivateKey privateKey, final String encodedText) {
		try {
			final Cipher cipher = Cipher.getInstance(RSA);
	 ...
	 * @return
	 */
	public static final String encodeByPublicKey(final RSAPublicKey publicKey,
			final String plainText) {
		try {
			final Cipher cipher = Cipher.getInstance(RSA);
			cipher.init(Cipher.ENCRYPT_MODE, publicKey);
			return URLEncoder.encode(new String(Base64.encodeBase64(cipher
					.doFinal(plainText.getBytes(UTF8)))), "utf-8");
		} catch (final Throwable t) {
			logger.error("", t);
			return null;
		}
	 ...
	 * @return
	 */
	public static final String decodeByPublicKey(final RSAPublicKey publicKey,
	 ...
	 * @return
	 */
	public static final RSAPublicKey gainRSAPublicKeyFromCrtFile(
	 ...
	 * @return
	 */
	public static final RSAPublicKey gainRSAPublicKeyFromPemFile(
		...
		try {
			...
			return (RSAPublicKey) (kf.generatePublic(new X509EncodedKeySpec(
	 ...
	 * @return
	 */
	public static String verifyPrivatePublicPair(
			final RSAPrivateKey rsaPrivateKey, final RSAPublicKey rsaPublicKey) {
		 ...
		 * 私钥加密，公钥解密
		 */
		final String encodedByPrivateKey = encodeByPrivateKey(rsaPrivateKey,
				input);
		final String decodedByPublicKey = decodeByPublicKey(rsaPublicKey,
				encodedByPrivateKey);
		if (!input.equals(decodedByPublicKey))
			 ...
			 * 公钥加密，私钥解密
			 */
			final String encodedByPublicKey = encodeByPublicKey(rsaPublicKey,
					input);
			final String decodedByPrivateKey = decodeByPrivateKey(
					rsaPrivateKey, encodedByPublicKey);
