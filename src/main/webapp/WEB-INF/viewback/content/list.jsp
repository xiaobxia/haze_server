...
<form id="pagerForm" onsubmit="return navTabSearch(this);" action="content/getContentPage?myId=${params.myId}" method="post">
				...
				<tr>
					...
					<td>
						...
						<c:forEach items="${CHANNEL}" var="channel">
							...
							<c:if test="${channel.key eq params.channelType }">selected="selected"</c:if>>${channel.value}</option>
						</c:forEach>
					</select>
					</td>
					<td>
